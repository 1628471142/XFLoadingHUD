...
static NSString * _timerName = @"XFLoadingAnimateTimerName";

@interface UIView (XFAnimatePrivate)
...
@implementation UIView (XFAnimatePrivate)
...
@implementation UIView (XFLoadingAnimate)

- (void)drawWordAnimationWithText:(NSString *)text{
    
    [self drawWordAnimationWithSize:CGSizeMake(0, 0) text:text font:nil startColor:nil endColor:nil position:XFLoadingPositionCenter];
...
- (void)drawWordAnimationWithSize:(CGSize)size
    ...
    [self drawWordAnimationWithSize:size text:text font:font startColor:nil endColor:nil position:XFLoadingPositionCenter];
...
- (void)drawWordAnimationWithSize:(CGSize)size
    ...
    [self drawWordAnimationWithSize:size text:text font:font startColor:startColor endColor:endColor position:XFLoadingPositionCenter];
...
- (void)drawWordAnimationWithSize:(CGSize)size
                        ...
                        startColor:(UIColor *)startColor
                          endColor:(UIColor *)endColor
                          position:(XFLoadingPosition)position{
    NSString * showText = text? : @"XF";
...
- (void)startTimerShowAnimateWithFrame:(CGRect)frame
    ...
    dispatch_source_set_timer(timer, dispatch_walltime(NULL, 0), [XFLoadingHUD shareInstance].customDuration * NSEC_PER_SEC, 0);
...
- (void)startDrawGraphWithFrame:(CGRect)frame text:(NSString *)text font:(UIFont *)font startColor:(UIColor *)startColor endColor:(UIColor *)endColor isErasure:(BOOL)isErasure{
    ...
    if ([XFLoadingHUD shareInstance].animateDirection == XFLoadingGraphDirectionEndToEnd) {
        ...
        pathAnimation.duration = [XFLoadingHUD shareInstance].customDuration;
...
//获取动画最终展示的Frame
- (CGRect)getShowFrame:(CGRect)frame position:(XFLoadingPosition)position{
    
    //动画位置
    XFLoadingPosition showPosition = position? : [XFLoadingHUD shareInstance].position;
